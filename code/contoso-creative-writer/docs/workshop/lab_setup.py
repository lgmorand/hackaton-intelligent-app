...
def track_origin_main():
    ...
    """
    subprocess.run(
        ['git', 'branch', 'main', '--set-upstream-to', 'origin/main'], check=True)
...
def azd_login(*, username: str = None, password: str = None, tenant: str = None, force: bool = False):
    ...
    if username and password:
        ...
        click.echo()
        click.echo(
            f"{style('IMPORTANT', fg='red', reverse=True)}: {style('DO NOT use your personal credentials for this step!', fg='red', underline=True)}")
        click.echo()
...
def export_variables():
    ...
    with open(env_path, 'w') as env_file:
        subprocess.run(['azd', 'env', 'get-values'],
                       stdout=env_file, check=True)
...
def set_env_vars():
    # Get the directory where the script is located and resolve .env path
    env_path = Path(__file__).parent.parent.parent / '.env'
    shell_profile = Path.home() / '.bashrc'  # or '.zshrc' for zsh users

    with open(env_path, 'r') as env_file, open(shell_profile, 'a') as profile_file:
        for line in env_file:
            if line.strip() and not line.startswith('#'):
                key, value = line.strip().split('=', 1)
                os.environ[key] = value
                profile_file.write(f'export {key}={value}\n')
...
def setup(username, password, azure_env_name, subscription, tenant, force, step):
    ...
    try:
        ...
        if step is not None:
            if not 1 <= step <= len(steps):
                raise click.BadParameter(
                    f"Step must be between 1 and {len(steps)}")
            start_step = step - 1
            ...
            if start_step >= len(steps):
                ...
                click.echo(
                    "Use --force to execute all steps from the beginning if needed.")
                return
        ...
        for index, entry in enumerate(steps):
            ...
            result = step_func(step_number=index + 1,
                               total_steps=len(steps), **step_params)
